...
import SignOutModal from '../components/common/Signout';
...
export default function Profile() {
  ...
  async function handleLogout() {
    try {
      ...
      removeSession(); // Clear local session
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row items-center gap-6">
            ...
            <button 
              ...
              onClick={triggerSignOut} // Use trigger here
            >
              <LogOut size={18} />
              Logout
            </button>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              {menuItems.map((item, index) => (
                <Link to={item.path} key={index}>
                  <button className="w-full flex items-center p-4 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-indigo-100 transition-all text-left group">
              ...
              {/* Mobile Logout Button inside the grid */}
              <button 
                onClick={triggerSignOut} // Use trigger here
                className="md:hidden flex items-center p-4 bg-white rounded-xl border border-red-50 text-red-600 shadow-sm"
              >
