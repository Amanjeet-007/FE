...
const Navbar = ({ filter = true }) => {
  ...
  function remove() {
    // 1. Check if the ref actually points to an element
    if (!all) {
      if (AllDiv.current) {
        AllDiv.current.style.transition = "1s";
        AllDiv.current.style.transform = "translateX(-100%)";
        AllDiv.current.style.opacity = "0";
      }
  ...
  return (
    <header className="sticky top-0 bg-primary border-b shadow-sm backdrop-blur-lg bg-opacity-95 z-50">
      <nav className='h-min'>
        ...
        <div className="menuIcon" onClick={() => setAll(el => !el)}>
        ...
        <div className="Alloption absolute h-[84vh] w-100 top-35 left-0 flex items-center overflow-y-scroll overflow-visible  flex-col" ref={AllDiv}>
          ...
          
          {
            All.map((el, i) => {
              return (
                <>
                  <p key={i} className="heading" style={{ color: "white" }}>{el.heading}</p>
                  {
                    el.option.map((a, i) => {
                      return (
                        <p className="namesInAll">{a.name}</p>
                      )
                    })
                  }
                </>)
